...
async def create_project(db: AsyncSession, tenant_id: uuid.UUID, data: ProjectCreate) -> Project:
    ...
    project = Project(tenant_id=tenant_id, project_no=project_no, name=data.name, description=data.description, inbox_email=inbox_email)
    db.add(project)
    await db.flush()
