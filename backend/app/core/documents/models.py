...
from sqlalchemy import Boolean, DateTime, String, Text, ForeignKey, Integer, UniqueConstraint
...
class ProjectDoc(Base, TimestampMixin, SoftDeleteMixin, TenantScopedMixin):
    """
    doc_no unique per tenant+project.
    category: HMS | MILJO | KVALITET | ANNET
    source_template_version_id: frozen library version this was imported from.
    """
    ...
    source_template_version_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("doc_template_versions.id", ondelete="SET NULL"), nullable=True)
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "project_id", "doc_no", name="uq_project_doc_no"),
    )
