...
class Nonconformance(Base, TimestampMixin, SoftDeleteMixin, TenantScopedMixin):
    ...
    source_type: "incident" | "checklist" | "manual" | "audit"
    """
    ...
    nc_type: Mapped[str] = mapped_column(String(50), nullable=False, default="nonconformance")  # nonconformance | observation | improvement
...
class CapaAction(Base, TimestampMixin, SoftDeleteMixin, TenantScopedMixin):
    ...
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    action_type: Mapped[str] = mapped_column(String(50), nullable=False, default="corrective")  # corrective | preventive
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="open")
    assigned_to: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    due_date: Mapped[str | None] = mapped_column(String(30), nullable=True)  # ISO date string
